...
def migrate_appointments_table():
    """Add symptoms column to appointments table if not exists"""
    conn = get_db()
    cursor = conn.cursor()
    ...
    try:
        ...
        print(f"Error adding symptoms column: {e}")
...
def init_db():
    ...
    # Create appointments table with symptoms column
    cursor.execute('''
    ...
    # Migrate appointments table to add symptoms column if needed
...
# ============ APPOINTMENTS (with symptoms support) ============
